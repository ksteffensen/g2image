...
$g2ic_version_text = '2.2';
$g2ic_version_array = array(2,2);
...
if(isset($_SESSION['g2ic_last_album_visited'])) {
	...
	list($ret, $g2ic_last_album) = GalleryCoreApi::getDefaultAlbumId();
}
...
function g2ic_get_img_info($img_id) {
	...
	if(!$error) {
		foreach ($items as $item) {
			...
			$fullsize_img = $urlGenerator->generateUrl(array('view' => 'core.DownloadItem', 'href' => "/gallery2/main.php", 'itemId' => $item->getid()));
			list($error, $thumbnails) = GalleryCoreApi::fetchThumbnailsByItemIds(array($item->getid()));
			if(!$error) {
				foreach($thumbnails as $thumbnail) {
					$thumbnail_src = $urlGenerator->generateUrl(array('view' => 'core.DownloadItem', 'href' => "/gallery2/main.php", 'itemId' => $thumbnail->getid()));
...
function g2ic_make_html_dir_menu(){
	global $gallery, $g2ic_current_album, $g2ic_dirs_titles, $g2ic_sortby, $g2ic_display_filenames, $g2ic_images_per_page,$g2ic_lang;
	...
	list($ret, $root_album) = GalleryCoreApi::getDefaultAlbumId();
	(!isset($_REQUEST['current_album'])) ? $selected = $g2ic_current_album : $selected = $_REQUEST['current_album'];
	$level = 0;
	$html .= get_album_hierarchy($root_album, $level, $selected);
	$html .= '            </select>'
	. '        </form>' . "\n"
	. '</div>' . "\n\n";

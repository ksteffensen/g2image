...
// ====( Version Info )
$g2ic_version_text = '3.0';
$g2ic_version_array = array(3,0);
...
function g2ic_get_gallery_items() {
	...
	if(!$error) {
		foreach ($albums as $album) {
			$album_info['url'] = $urlGenerator->generateUrl(array('view' => 'core.ShowItem', 'itemId' => $album->getid()), array('forceServerRelativeUrl' => true));
...
function g2ic_get_item_info($item_id) {
	...
	if(!$error) {
		foreach ($items as $item) {
			...
			$item_info['fullsize_img'] = $urlGenerator->generateUrl(array('view' => 'core.DownloadItem', 'itemId' => $item->getid()), array('forceServerRelativeUrl' => true));
			list($error, $thumbnails) = GalleryCoreApi::fetchThumbnailsByItemIds(array($item->getid()));
			if(!$error) {
				foreach($thumbnails as $thumbnail) {
					$item_info['thumbnail_src'] = $urlGenerator->generateUrl(array('view' => 'core.DownloadItem', 'itemId' => $thumbnail->getid()), array('forceServerRelativeUrl' => true));
					$item_info['image_url'] = $urlGenerator->generateUrl(array('view' => 'core.ShowItem', 'itemId' => $item->getid()), array('forceServerRelativeUrl' => true));
...
function g2ic_make_html_alignment_select(){
	GLOBAL $g2ic_default_alignment, $g2ic_custom_class_1, $g2ic_custom_class_2, $g2ic_custom_class_3, $g2ic_custom_class_4;

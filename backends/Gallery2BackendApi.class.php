...
class Gallery2BackendApi{
	...
	/** TODO Make sure that all of the URLs generated are correct, then delete this.
	...
	function g2ic_get_item_info($item_id) {
		...
		$thumbnails = $g2obj->fetchThumbnailsByItemId($item_id);
		...
		if (!$item->getCanContainChildren()) {
			...
			if (preg_match('/image/', $item_mime_type)) {
				...
				$resizes_array = $g2obj->fetchResizesByItemId($item_id);
